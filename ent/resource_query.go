...
func (rq *ResourceQuery) QueryOwner() *GroupQuery {
	...
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.M2O, true, resource.OwnerTable, resource.OwnerColumn),
		)
...
func (rq *ResourceQuery) IDs(ctx context.Context) ([]uuid.UUID, error) {
	var ids []uuid.UUID
...
func (rq *ResourceQuery) WithOwner(opts ...func(*GroupQuery)) *ResourceQuery {
	query := &GroupQuery{config: rq.config}
...
func (rq *ResourceQuery) sqlAll(ctx context.Context) ([]*Resource, error) {
	...
	if query := rq.withOwner; query != nil {
		...
		for i := range nodes {
			if nodes[i].group_resources == nil {
				continue
			}
			fk := *nodes[i].group_resources
		...
		for _, n := range neighbors {
			...
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "group_resources" returned %v`, n.ID)
			}
			for i := range nodes {
